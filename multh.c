...
void *new_func(void *para)
{
    ...
    while(SYNC_FLAG == 1)//全局标志，多线程全部初始化完毕，main函数才会打开这个标志，多线程才开始执行pwrite
    {
        ...
        //if(DEBUG) 
        printf("thread[%d] threads-iosize:%dK,thread-avg-time:%f\n", ptr->icore+1, ptr->io_size/1024, sum_ths_time[ptr->icore]);
    }
}
